...
namespace SFrame
{
    ...
    public class AStarMgr : BaseManager<AStarMgr>
    {
        ...
        public void InitMapInfo(int w, int h)
        {
            ...
            for ( int i = 0; i < w; ++i )
            {
                for( int j = 0; j < h; ++j )
                {
                    ...
                    AStarNode node = new AStarNode(i, j, Random.Range(0, 100) < 20 ? ENodeType.Stop : ENodeType.Walk);
                    nodes[i, j] = node;
                }
            }
        }
        ...
        public List<AStarNode> FindPath(Vector2 startPos, Vector2 endPos)
        {
            ...
            if (start.type == ENodeType.Stop ||
                end.type == ENodeType.Stop)
            {
        ...
        private int SortOpenList(AStarNode a, AStarNode b)
        {
            if (a.f > b.f)
                return 1;
            else if (Mathf.Approximately(a.f,b.f))
                return 1;
            else
                return -1;
        }
        ...
        private void FindNearlyNodeToOpenList(int x, int y, float g, AStarNode father, AStarNode end)
        {
            ...
            if (node == null||
                node.type == ENodeType.Stop ||
                _closeList.Contains(node) ||
                _openList.Contains(node) )
                return;
            //计算f值
            //f = g + h
            //记录父对象
